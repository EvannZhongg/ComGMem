...
class StorageConfig(BaseModel):
    backend: str = "sqlite"
    path: str = "runs/comgmem/memory.sqlite3"


class VectorStoreConfig(BaseModel):
    backend: str = "qdrant"
    path: str = "runs/comgmem/vector_index"
    collection_name: str = "comgmem_memory"
...
def _find_project_root(path: Path) -> Path:
    ...
    for candidate in [start, *start.parents]:
        if (candidate / "pyproject.toml").exists() and (candidate / "comgmem").exists():
            return candidate
    return start
