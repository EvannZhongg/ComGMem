...
from comgmem.errors import ConfigError, StoreError
from comgmem.schema import EdgeCluster, HyperEdge, LocalTriple, MemoryNode, Message
    ...
    def client(self) -> Any:
        if self._client is None:
            try:
                ...
                raise ConfigError("Install comgmem[vector] to use the Qdrant vector store.") from exc
...
def make_vector_point_id(namespace: str, item_type: str, item_id: str | None = None) -> str:
    if item_id is None:
        item_id = item_type
        item_type = "node_local_graph"
    return str(uuid5(NAMESPACE_URL, f"comgmem:{namespace}:{item_type}:{item_id}"))
