...
- `metadata.periphery_edges` / `metadata.periphery_nodes`：只由 Top K2 core edges 所属 cluster 带出的 sibling context，并受 `recall.cluster_periphery_edge_limit` / `recall.cluster_periphery_node_limit` 限制；截断前会优先保留更新 turn 更近的 sibling edges 和 periphery nodes。每条 sibling edge payload 包含 `description/node_ids/time/relative_time/edge_metadata/nodes`，其中 `nodes` 同样携带受 `recall.node_triple_limit` 限制并按 edge scope/source turn 优先排序的 active triples 与相对 turn 距离。periphery 不参与 core edge 排名，也不会继续触发新的 cluster 扩散。
