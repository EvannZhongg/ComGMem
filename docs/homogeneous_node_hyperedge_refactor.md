...
- LLM 输出 `nodes / edge_summaries / sources`，不再输出 `entities / events / assertions / sources`。
...
- HyperEdge 核心只保留 `description + node_ids + source refs/metadata`。
...
| 一次抽取 | `## 5. 一次抽取，系统组装` | 抽取输出改为 `nodes / edge_summaries / sources`。 |
...
- `ExtractedAssertion.polarity` 删除或仅为旧数据兼容保留。
- `HyperEdge.polarity` 删除或仅为旧数据库兼容保留。
- `HyperEdge.roles` 不再作为核心字段；如为兼容保留，新的写入流程不再依赖。
...
- Prompt 输出 shape 改为 `nodes / edge_summaries / sources`。
- 删除 prompt 中 `entities/events/assertions`、`polarity`、`assertions as single carrier` 的规则。
- `_strict_json_shape()` 改为要求 `nodes/edge_summaries/sources`。
- `normalize_extraction_payload()` 改为归一化：
  ...
  - source refs 必须可解析
- 增加向后兼容转换层可选：旧 `entities/events/assertions` payload 可以临时转换为新 `nodes/edge_summaries`，用于平滑迁移测试。
...
MemoryExtraction.nodes
  ...
  -> build HyperEdges from edge summaries
  -> build EdgeClusters
...
- `edge_merge.md` 不再要求 relation/roles/polarity 兼容；改为 description、members、source/time、inferred metadata 只作为辅助信号。
...
- `nodes.local_graph_json` 中旧 `attributes/roles` 需要兼容读取，但新写入不产生。
...
- 需要数据库迁移策略：
  ...
  - 若要兼容旧库，需要增加 schema version 和 migration。
...
- 新增 schema 测试：
  - `nodes/edge_summaries/sources` payload 可解析。
  - LLM 不输出 `edge_type/relation/polarity/roles`。
...
| 1 | Schema 引入新结构 | 未开始 | 增加 `ExtractedNode/ExtractedEdgeSummary`，保留旧 schema 兼容。 | `schema.py` |
| 2 | 抽取 prompt 与 parser 重构 | 未开始 | LLM 输出 `nodes/edge_summaries/sources`。 | `memory_extraction.md`, `extraction.py` |
| 3 | NodeBuilder 与 LocalGraphBuilder 重构 | 未开始 | 统一 `build_node`，删除按 label 写死 triples 的主路径。 | `node_builder.py`, `local_graph_builder.py` |
| 4 | GraphAssembler 重构 | 未开始 | 按 refs 组装 nodes 和 HyperEdges。 | `assembly.py`, `hyperedge_builder.py` |
| 5 | 存储兼容与索引调整 | 未开始 | 新写入不依赖 polarity/roles；兼容旧库读取。 | `sqlite_store.py`, `vector_store.py` |
...
为降低风险，建议不要一次性删除旧路径，而是使用短期双轨：

1. 先在 schema 层新增新结构，旧结构仍可解析。
2. 抽取 prompt 切换到新结构。
3. parser 支持旧 payload 自动转换为新 payload。
4. assembly 内部只消费新结构。
5. 测试稳定后，再删除旧 `assertions` 主路径。
...
兼容转换只用于迁移，不应继续作为长期架构入口。

## 6. 当前进度

- 已完成：`docs/hypergraph_memory_architecture.md` 已调整为同构节点、`nodes/edge_summaries/sources`、description-only HyperEdge、可选 inferred metadata 的设计方向。
